...
In-paper anchor: `paper/GOLDBACH_CONJECTURE_PREPRINT.md` (`GB_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `prove the binary Goldbach statement by routing admissible circle-method states through major-arc coercivity, minor-arc capture, compactness, rigidity, prime-pair transfer, and strict coherence`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`GB_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and arc windows do not collapse.
This is the first-failure capture contribution to `M_GB`.
...
If near-failure extraction and no-Zeno spacing hold, then `GB_G3` prevents hidden drift before rigidity is applied.
