...
In-paper anchor: `paper/GOLDBACH_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `prove the binary Goldbach statement by routing admissible circle-method states through major-arc coercivity, minor-arc capture, compactness, rigidity, prime-pair transfer, and strict coherence`.
...
- endpoint lock: `prime_pair_transfer`.
...
2. the endpoint-transfer lock `prime_pair_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_GB` remains positive.
...
Payload: verify `prime_pair_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_GB` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `prime_pair_transfer`
