...
Canonical Lane research workspace for a flagship problem in additive number theory:
proving that every even integer greater than 2 is a sum of two primes through a major-arc/minor-arc closure architecture.

## Main Manuscript

- [paper/GOLDBACH_CONJECTURE_PREPRINT.md](paper/GOLDBACH_CONJECTURE_PREPRINT.md)
- [paper/CANONICAL_ROUTING_INDEX.md](paper/CANONICAL_ROUTING_INDEX.md)

## Structure

- `paper/`
  - `GOLDBACH_CONJECTURE_PREPRINT.md`
...
1. Theorem chain first: read `paper/GOLDBACH_CONJECTURE_PREPRINT.md`.
...
Current GB runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/gb_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/gb_closure_guard.py`.
...
- Manuscript target: [paper/GOLDBACH_CONJECTURE_PREPRINT.md](paper/GOLDBACH_CONJECTURE_PREPRINT.md)
