#!/usr/bin/env python3
"""Canonical-lane closure guard for the goldbach conjecture workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['GB_G1', 'GB_G2', 'GB_G3', 'GB_G4', 'GB_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['GB_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['GB_GM'] = 'PASS' if gm_pass else 'FAIL'
    ...
    blockers['GB_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['GB_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
